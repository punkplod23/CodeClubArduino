...
We've used the BBC micro:bit previously and now we would like to introduce another. 

Micro Controllers are everywhere to home appliances to industrial machinaey to traffic lights.

# What is an Arduino?
An Arduino is a microcontroller. We're going to use it to control our robot. Arduino are coded in C.

# Why use a micro contoller over a computer.
1. Cost - microcontrollers are cheaper than computers.
2. Power consumption - micro controllers require less power than computers.
3. Size - microcontrollers are smaller and lighter than computers so useful for vehicles.  

# Why Arduino
1. Cost (around: £1.71 per board)
...
2. C is the godfather of modern day programming languages.
...
5. Dennis Ritchie passed away in the same year as Steve Jobs. There would not have been a Steve Jobs without Dennis Ritchie. Ironically, more people know the name Steve Jobs than Dennis Ritchie.
...
2. In Arduino our programs are reffered to as sketches. I think its a silly name as well.
...
# Arduino Micro board.
1. We will be using the Arduino Micro board. A break down ot its pin setup is below.
...
# Our first Sketch
1. We are going to use the serial monitor back and forth to create a simple feedback loop over serial.
...
11. Where you can type in some text in this screen type in some numbers and hit send.
12. Did it add 10 to the number you entered.
13. We created a small calculator.
